/**
 * Virtual Nikkah Bulletproof Real-Time Application Engine
 * Supports WebRTC PeerJS + BroadcastChannel + LocalStorage Event Fallback
 */

// Dynamic Room Identifier (Default or URL hash)
let ROOM_ID = 'nikkah-room-' + (window.location.hash ? window.location.hash.replace('#', '') : '7777');
...
// 1. BroadcastChannel Engine (Instant local cross-tab sync)
let broadcastChannel = null;
try {
    broadcastChannel = new BroadcastChannel('virtual_nikkah_sync_channel');
    broadcastChannel.onmessage = (event) => {
        handleIncomingData(event.data);
    };
} catch(e) {}

// 2. LocalStorage Polling Engine (Bulletproof fallback)
window.addEventListener('storage', (event) => {
    if (event.key === 'virtual_nikkah_event' && event.newValue) {
        try {
...
function sendRealtimeData(data) {
    ...
    if (state.conn && state.conn.open) {
        try { state.conn.send(data); } catch(e) {}
    }
    // B. BroadcastChannel
    if (broadcastChannel) {
    ...
    try {
        localStorage.setItem('virtual_nikkah_event', JSON.stringify({ ...data, _ts: Date.now() }));
    } catch(e) {}
}

// 3. PeerJS Connection Handlers
function startGroomPeer() {
    const groomId = `${ROOM_ID}-groom`;
    const brideId = `${ROOM_ID}-bride`;

    try {
        state.peer = new Peer(groomId, { debug: 1, secure: true });
        
        state.peer.on('open', () => {
            elements.waitingStatusMsg.textContent = 'Groom Online 🟢 Waiting for Dulhan to tap Join on her phone...';
            updateLiveStatus(true, 'Waiting for Dulhan... 🟢');
            attemptConnectToTarget(brideId);
        });
        ...
        state.peer.on('error', (err) => {
            console.warn('Groom peer error:', err);
            // If ID busy, retry with random suffix
            if (err.type === 'unavailable-id') {
                ROOM_ID = ROOM_ID + '-' + Math.floor(Math.random() * 100);
                startGroomPeer();
            }
        });
    } catch(e) {
...
function startBridePeer() {
    ...
    try {
        state.peer = new Peer(brideId, { debug: 1, secure: true });

        state.peer.on('open', () => {
            elements.waitingStatusMsg.textContent = 'Dulhan Online 🟢 Connecting to Dulha...';
            updateLiveStatus(true, 'Connecting to Dulha... 🟢');
            attemptConnectToTarget(groomId);
        });
        ...
        state.peer.on('error', (err) => {
            console.warn('Bride peer error:', err);
            attemptConnectToTarget(groomId);
        });
    } catch(e) {
        onPeerConnected();
...
function attemptConnectToTarget(targetId) {
...
elements.btnCreateRoom.addEventListener('click', () => {
    ...
    // Auto-advance to ceremony after 3 seconds even if peer is waiting
    setTimeout(() => {
...
elements.btnConfirmJoin.addEventListener('click', () => {
    ...
    setTimeout(() => {
        if (!state.isConnected) {
            sendRealtimeData({ type: 'BRIDE_JOINED', brideName: state.yourName });
            startCeremonyStage();
        }
    }, 3000);
});
...
function startCeremonyStage() {
    ...
    if (state.role === 'groom') {
        updateQaziSpeech(`Aap dono online active connect ho chuke hain! Dulha <strong>${state.yourName}</strong>, 'Make Ijab (Offer)' button click karain.`);
        elements.groomControls.classList.remove('hidden-box');
        elements.brideControls.classList.add('hidden-box');
    } else {
        updateQaziSpeech(`Aap dono online active connect ho chuke hain! Dulha <strong>${state.partnerName}</strong> abhi Nikkah ka Ijab (Offer) karay ga...`);
        elements.groomControls.classList.add('hidden-box');
        elements.brideControls.classList.add('hidden-box');
    }
}
